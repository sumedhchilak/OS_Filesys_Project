...
process_execute (const char *file_name)
{
  ...
  if (new_child_thread_tid == TID_ERROR)
  ...
  else
    {
      ...
      sema_down (&new_child_thread->load_sema);
      if (new_child_thread->load_status == -1)
        new_child_thread_tid = TID_ERROR;
...
process_exit (void)
{
  ...
  // Child is about to exit, parent can unblock
